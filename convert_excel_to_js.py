...
try:
    ...
    for _, row in main_df.iterrows():
        ...
        if isinstance(adm_date, pd.Timestamp):
            ...
            if adm_date.strftime('%Y-%m-%d') == '2026-07-31' or adm_date.strftime('%d-%m-%Y') == '31-07-2026':
                is_highlighted = True
        else:
            adm_date_str = str(adm_date).split(" ")[0] if adm_date else ""
            if "2026-07-31" in adm_date_str or "31-07-2026" in adm_date_str:
                is_highlighted = True
    ...
    for _, row in tc_df.iterrows():
        ...
        if isinstance(exit_date, pd.Timestamp):
            ...
            if exit_date.strftime('%Y-%m-%d') == '2026-07-31' or exit_date.strftime('%d-%m-%Y') == '31-07-2026':
                is_highlighted = True
        else:
            exit_date_str = str(exit_date).split(" ")[0] if exit_date else ""
            if "2026-07-31" in exit_date_str or "31-07-2026" in exit_date_str:
                is_highlighted = True
